...
{ // Procura na tabela e retorna se esta presente
    int h = hash(name, scope);
    BucketList l = hashTable[h];
...
{ // Procura na tabela e retorna a quantidade de parametros da funcao
    int h = hash(name, scope);
    BucketList l = hashTable[h];
    while ((l != NULL) && (strcmp(name, l->name) != 0) && (strcmp(scope, l->scope) != 0))
    {
...
{ // Procura na tabela e retorna o tamanho do vetor
    int h = hash(name, scope);
    BucketList l = hashTable[h];
    while ((l != NULL) && (strcmp(name, l->name) != 0) && (strcmp(scope, l->scope) != 0))
    {
...
void printSymTab(FILE *tabSim)
{
    ...
    fprintf(tabSim, "quantum         global          function        void             --                \n");
    fprintf(tabSim, "program         global          function        void             --                \n");
    for (i = 0; i < SIZE; ++i)
    {
        if (hashTable[i] != NULL)
        {
