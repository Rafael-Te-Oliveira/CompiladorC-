...
void decimalToBinary(int number, int bitNo, FILE *binFile)
...
{
    if (inst.instKind == BEQ || inst.instKind == BNE || inst.instKind == BLT || inst.instKind == BGT || inst.instKind == BLE || inst.instKind == BGE)
    {                                                             // Se a instrucao for branch
